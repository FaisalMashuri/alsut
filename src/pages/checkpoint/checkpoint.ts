...
export class CheckpointPage {
    ...
    constructor(
        ...
        console.log(this.clusterCode);
        this.mapUrl = this._sanitizer.bypassSecurityTrustResourceUrl('https://guardtour.alam-sutera.com/cluster/' + this.clusterCode + '/webview');
        this.fileLogs = '';
        this.mqttData = {
            host: '43.251.98.16',
            currentTopic: 'api',
    ...
    uploadFileLog() {
        return new Promise((resolve, reject) => {
            ...
            console.log(fileTransfer.upload(this.file.externalDataDirectory + this.fileLogs, this.host + '/upload_log', options))
            fileTransfer.upload(this.file.externalDataDirectory + this.fileLogs, this.host + '/upload_log', options)
            .then((data) => {
                console.log("success upload file log")
                resolve('success');
            }, (err) => {
                console.log(err)
                swal({
